...
Out current estimate $\beta_{t}$ measure impacts of sub-MW worker % on % change of $y_{it}$:
\begin{equation*}
% change in y_{it} = \alpha_{t}+\beta_{t}FA_{i}+\bfgamma'\bfx_{it}+\epsilon_{it}.
\end{equation*}

### If $FA_{i}=1$, all workers are sub-MW workers, and $\beta_{t}$ measures the impacts of (% change in) MW.
<p style="margin-bottom:2cm;">
### If $FA_{i}=.5$, 1/2 of workers are sub-MW workers, and $\beta_{t}$ measures the impacts of (%/.5 change in) MW.
<p style="margin-bottom:2cm;">
### $\hat{\beta}_{t}/FA_{i}$ gives an elasitcity of MW on outcome $y_{it}$. 
