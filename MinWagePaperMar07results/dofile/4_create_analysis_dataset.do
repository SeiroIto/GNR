...
foreach dataset in unbalanced balanced survivors {
		...
		drop if c_type == 2
		cap drop _merge 
		merge 1:1 taxrefno taxyear using "Z:\Master Data\Auxillary capital stock\Imputed_capital_stock_beta.dta",  keepusing(pi_iv_fixed_pd_10) keep(master matched)
		drop _merge 
	...
	*ADD THE RAINFALL VARIABLE 
		...
		preserve 
		use "$saveaddress_data\Annual rainfall by province in line with taxyears.dta", clear 
		...
		merge m:1 taxyear province using `rainfall'
	...
	*GENERATE FIRM-SIZE DUMMY VARIABLES 
		gen micro_firm_dummy = 0 
		replace micro_firm_dummy = 1 if c_type_adj == 1
		
		gen small_firm_dummy = 0 
		replace small_firm_dummy = 1 if c_type_adj == 2
		
		gen medlarg_firm_dummy = 0 
		replace medlarg_firm_dummy = 1 if c_type_adj == 3
	...
	*LOG | EMPLOYEE TRAINING EXPENDITURE 
	...
	local keep_log = "ltrain lnon_wage_labcosts lawage llabcost_pe llabcost_capcost loprofit lmaterials l_labcost  loprofits_pe ltfp lcap_lab lrevenue lrev_pe "
